...
impl RationalExtensions for BigRational {
        ...
        match ang {
            ...
            AngleUnit::Radians => {
                if self.denom().is_one() {
                    ...
                    format!("{}{}", self.to_nice_string(subsuperscript), MathsSymbols::Pi)
                } else {
                    format!("{}/{}{}", if self.numer().is_one() {
                        "".to_string()
                    } else {
                        self.numer().to_string()
                    }, MathsSymbols::Pi, self.denom())
                }
            }
            AngleUnit::Gradians => { todo!() }
        }
    }
    ...
    fn reduced_arg(&self) -> BigRational {
        if self >= &BigRational::new((-1).into(), 2.into()) && self < &BigRational::new(3.into(), 2.into())
        {
            return self.clone();
        }
